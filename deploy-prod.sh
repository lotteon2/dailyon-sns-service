...
if kubectl rollout status deployment/sns-deployment -n prod | grep -q "successfully rolled out"; then
  echo "Success to create new kubernetes deployment resources"

  kubectl annotate deployment/sns-deployment kubernetes.io/change-cause="Change image to ${IMAGE_TAG}"
  ...
  kubectl rollout undo deployment/sns-deployment -n prod
  if kubectl rollout status deployment/sns-deployment -n prod | grep -q "successfully rolled out"; then
    echo "Rollback was successful"

    kubectl annotate deployment/sns-deployment kubernetes.io/change-cause="Rollback to previous image"
  else
